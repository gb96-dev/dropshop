...
class SellerDashboardRefreshServiceTest {
  ...
  void setUp() {
    ...
    given(redisLockService.executeWithLock(anyString(), any())).willAnswer(
  ...
  void refreshForOrder_saveAggregate() {
    ...
    given(sellerDashboardMetricsRepository.calculateDailyAggregate(77L, order.getCreatedAt().toLocalDate()))
        .willReturn(aggregate);
    given(sellerDashboardDailyRepository.findBySellerIdAndStatDate(77L, order.getCreatedAt().toLocalDate()))
  ...
  void refreshForOrder_deleteWhenEmpty() {
    SellerDashboardDaily existing = SellerDashboardDaily.create(
        77L,
