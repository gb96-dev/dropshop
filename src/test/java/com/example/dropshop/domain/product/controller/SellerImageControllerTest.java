...
class SellerImageControllerTest {
  ...
  @Test
  @DisplayName("SELLER 권한이 없으면 Presigned URL 발급이 거부된다")
  @WithAnonymousUser
    // 인증 없는 사용자
  void issuePresignedUrl_unauthorized_forbidden() throws Exception {
    mockMvc.perform(post("/api/sellers/images/presigned-url")
        .with(csrf())
        .contentType(APPLICATION_JSON)
        .content("{}"))
      .andExpect(status().isForbidden()); // 또는 .isUnauthorized()
  }
}
