...
class SellerControllerTest {
  ...
  void applySeller_Success() throws Exception {
    ...
    mockMvc
        .perform(
            post("/api/sellers/apply").contentType(MediaType.APPLICATION_JSON).content(jsonContent))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.brandName").value("드랍숍"));

    // then: ArgumentCaptor를 사용하여 실제 전달된 데이터 검증
    ArgumentCaptor<SellerApplyRequest> requestCaptor =
        ArgumentCaptor.forClass(SellerApplyRequest.class);
  ...
  void applySeller_Fail_EmptyBrandName() throws Exception {
    ...
    mockMvc
        .perform(
            post("/api/sellers/apply").contentType(MediaType.APPLICATION_JSON).content(invalidJson))
  ...
  void getMySellerStatus_Success() throws Exception {
    ...
    mockMvc
        .perform(get("/api/sellers/me/status").accept(MediaType.APPLICATION_JSON))
        .andExpect(status().isOk())
