...
/**
 * 통계 조회 전용 리포지토리 (QueryDSL).
 * sellerId 가 null 이면 전체(관리자용), 있으면 해당 판매자만(판매자용).
 */
@Repository
...
public class StatisticsRepository {
     ...
     * 판매 추이 조회 (날짜별 집계).
     */
    public List<SalesTrendResponse> findSalesTrend(LocalDateTime from, LocalDateTime to,
                                                    Long sellerId) {
        DateTemplate<LocalDate> orderDate = Expressions.dateTemplate(
                LocalDate.class, "DATE({0})", order.createdAt);
        ...
        NumberTemplate<BigDecimal> revenueSum =
                Expressions.numberTemplate(BigDecimal.class,
                        "SUM({0} * {1})", orderItem.salePriceSnapshot, orderItem.quantity);

        return queryFactory
                .select(Projections.constructor(
                        SalesTrendResponse.class,
                        orderDate,
                        quantitySum,
                        revenueSum
                ))
     ...
     * 인기 상품 조회 (판매량 기준 정렬).
     */
    public List<PopularProductResponse> findPopularProducts(LocalDateTime from, LocalDateTime to,
