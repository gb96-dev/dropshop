 ...
 * </ul>
 *
 * <p>DAU는 HyperLogLog(PFADD)를 사용해 동일 이메일의 중복 로그인을 무시하며, 오차율 약 0.81% 이내의 근사 집계를 제공한다. 모든 키 TTL은
 * 48시간으로 설정한다.
 */
@Slf4j
@Component
...
public class UserActivityKafkaConsumer {
   ...
   */
  private static final RedisScript<Long> SIGNUP_DEDUP_SCRIPT =
      RedisScript.of(
  ...
  /**
   * 로그인 이벤트 수신 → Redis HyperLogLog에 이메일 추가(DAU 고유 집계). 동일 이메일의 중복 로그인은 PFADD가 무시하므로 DAU가 중복 집계되지
   * 않는다.
   */
  @KafkaListener(
      topics = TOPIC_USER_LOGIN,
      groupId =
          "#{T(com.example.dropshop.common.constant.kafka.group.KafkaGroups).USER_LOGIN_GROUP_NAME}",
  ...
  /**
   * 회원가입 이벤트 수신 → Redis 신규 가입자 카운터 증가. 날짜별 Set으로 이메일 dedupe 후 최초 수신 시에만 카운터를 증가시켜 Kafka
   * at-least-once 재전달로 인한 중복 집계를 방지한다.
   */
  @KafkaListener(
      topics = TOPIC_USER_SIGNUP,
      groupId =
          "#{T(com.example.dropshop.common.constant.kafka.group.KafkaGroups).USER_SIGNUP_GROUP_NAME}",
