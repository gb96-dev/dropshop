 ...
 * </ul>
 *
 * <p>seen Set + 카운터 + 매출 증가를 Lua 스크립트로 원자적으로 실행해 Kafka at-least-once 재전달로 인한 중복 집계를 방지한다.
 */
@Slf4j
@Component
...
public class PaymentStatisticsKafkaConsumer {
   ...
   *
   * <p>PFADD는 동일 값에 대해 idempotent이지만, SADD 성공 후 프로세스 중단 시 paymentId가 이미 seen Set에 있어 재처리가 스킵되면
   * PFADD가 영구 유실된다. 따라서 PFADD도 Lua 스크립트 내에서 원자적으로 실행한다.
   */
  private static final RedisScript<Long> PAYMENT_STATS_SCRIPT =
      RedisScript.of(
  ...
  @KafkaListener(
      ...
      groupId =
          "#{T(com.example.dropshop.common.constant.kafka.group.KafkaGroups).PAYMENT_STATS_GROUP_NAME}",
  ...
  public void handlePaymentCompleted(PaymentStatusChangedEvent event) {
    ...
    if (event.getPaymentStatus() != PaymentStatus.COMPLETED) {
      log.warn(
          "[Kafka Consumer] COMPLETED 아닌 결제 이벤트 수신 - 스킵. status: {}, paymentId: {}",
    ...
    String buyerUserIdArg =
        event.getBuyerUserId() != null ? String.valueOf(event.getBuyerUserId()) : "";
    ...
    log.info(
        "[Kafka Consumer] 결제 통계 업데이트 완료 - yearMonth: {}, paymentId: {}, amount: {}",
        yearMonth,
        event.getPaymentId(),
        event.getAmount());
  }
