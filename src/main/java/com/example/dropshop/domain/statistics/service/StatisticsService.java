...
public class StatisticsService {
     ...
     * 인기 상품 조회 (판매량 기준).
     *
     * 날짜 필터와 판매자 필터가 모두 없는 경우 Redis Z셋(실시간 누적)에서 우선 조회한다.
     * 그 외(날짜/판매자 조건이 있거나 Redis 데이터 없음)는 DB에서 조회한다.
     *
    ...
    public List<PopularProductResponse> getPopularProducts(LocalDateTime from, LocalDateTime to,
        ...
        if (from == null && to == null && sellerId == null
                && popularProductRedisService.hasData()) {
    ...
    /**
     * Redis Z셋에서 탑 N 인기 상품을 조회한다.
     * score(누적 판매량)를 totalQuantity 로, totalAmount 는 0 으로 반환한다.
     */
    private List<PopularProductResponse> getPopularProductsFromRedis(int limit) {
        ...
        // productId → 누적 판매량 매핑
        Map<Long, Long> scoreMap = topTuples.stream()
        ...
        return products.stream()
                ...
                .sorted(Comparator.comparingLong(
                        r -> -scoreMap.getOrDefault(r.getProductId(), 0L)
                ))
