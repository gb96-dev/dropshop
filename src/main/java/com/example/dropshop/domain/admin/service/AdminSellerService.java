...
/**
 * 관리자용 판매자 관리 서비스. - 전체 판매자 목록 조회 - 승인 대기(PENDING) 판매자 목록 조회 - 판매자 승인 (PENDING → APPROVED) - 판매자 정지
 * (APPROVED → SUSPENDED)
 */
@Service
...
public class AdminSellerService {
  ...
  public List<SellerResponse> getAllSellers() {
    return sellerRepository.findAll().stream().map(SellerResponse::new).toList();
  }
