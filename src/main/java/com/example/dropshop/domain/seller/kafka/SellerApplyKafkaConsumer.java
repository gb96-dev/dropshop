...
public class SellerApplyKafkaConsumer {
   ...
   *
   * <p>Redis Lua 스크립트는 서버 측에서 원자적으로 실행되므로 SADD 성공 후 LPUSH 전에 프로세스가 죽어도 이벤트가 유실되지 않는다.
   */
  private static final RedisScript<Long> SELLER_APPLY_ENQUEUE_SCRIPT =
      RedisScript.of(
  ...
  @KafkaListener(
      ...
      groupId =
          "#{T(com.example.dropshop.common.constant.kafka.group.KafkaGroups).SELLER_APPLY_GROUP_NAME}",
  ...
  public void handleSellerApply(SellerAppliedEvent event) {
    ...
    if (enqueued == null || enqueued == 0L) {
      log.warn("[Kafka Consumer] 중복 판매자 신청 이벤트 감지 - 스킵. 사업자번호: {}", maskBusinessNo(businessNo));
      return;
    }

    log.info(
        "[Kafka Consumer] 판매자 신청 이벤트 수신 - email: {}, 업체: {}, 사업자번호: {}",
  ...
  /** 사업자 번호를 마스킹한다. 예) 123-45-67890 → 123-**-***** 1234567890 → 123******* */
