 ...
 *
 * <p>각 메서드는 브로커 ack를 최대 {@value #SEND_TIMEOUT_SECONDS}초 동안 동기 대기한다. 발행에 실패하면 {@link
 * KafkaPublishException}을 던져 호출부 트랜잭션이 롤백되도록 한다.
 */
@Slf4j
@Component
...
public class EventKafkaProducer {
  ...
  public void publishUserLogin(UserLoginEvent event) {
    ...
    send(TOPIC_USER_LOGIN, hashEmail(event.getEmail()), event, "로그인 이벤트 발행 실패 - email: " + masked);
  ...
  public void publishUserSignup(UserSignupEvent event) {
    String masked = maskEmail(event.getEmail());
    send(
        TOPIC_USER_SIGNUP, hashEmail(event.getEmail()), event, "회원가입 이벤트 발행 실패 - email: " + masked);
  ...
  public void publishSellerApply(SellerAppliedEvent event) {
    ...
    send(
        TOPIC_SELLER_APPLY,
        hashEmail(event.getEmail()),
        event,
        "판매자 신청 이벤트 발행 실패 - email: " + masked);
    log.info("[Kafka] 판매자 신청 이벤트 발행 완료 - email: {}, 업체: {}", masked, event.getCompanyName());
  }
  ...
  /** Kafka 발행 후 브로커 ack를 동기 대기한다. 실패 시 로그를 남기고 {@link KafkaPublishException}을 던진다. */
  ...
  /**
   * 이메일을 SHA-256으로 해싱해 파티션 키로 사용한다. 원본 이메일을 Kafka 브로커/로그에 노출하지 않으면서 동일 유저의 메시지가 같은 파티션에 순서대로 도달하도록
   * 결정론적 키를 보장한다.
   */
